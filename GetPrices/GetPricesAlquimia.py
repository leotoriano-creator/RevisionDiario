...
if GOOGLE_SERVICE_ACCOUNT_JSON and not SERVICE_ACCOUNT_FILE.exists():
    try:
        service_account_data = json.loads(GOOGLE_SERVICE_ACCOUNT_JSON)

        SERVICE_ACCOUNT_FILE.write_text(
            json.dumps(service_account_data),
            encoding="utf-8",
        )

        print(
            f"service_account.json creado desde GOOGLE_SERVICE_ACCOUNT_JSON en: "
            f"{SERVICE_ACCOUNT_FILE}"
        )

    except Exception as e:
        raise RuntimeError(
            f"No pude crear service_account.json desde GOOGLE_SERVICE_ACCOUNT_JSON: {e}"
        )
...
PRICE_CANDIDATES_BY_SHEET = {
    ...
    "CEDEAR": ["Precio", "Precio ARS", "precio"],
...
def get_drive_service():
    if not SERVICE_ACCOUNT_FILE.exists():
        raise FileNotFoundError(
            ...
            "En local debe existir service_account.json. "
            "En Railway debe existir la variable GOOGLE_SERVICE_ACCOUNT_JSON."
        )
...
def find_latest_ultimo_cierre(service):
    """
    Busca todos los archivos accesibles llamados ultimo_cierre.xlsx
    y elige el más recientemente modificado.
    """
...
def find_header_row(raw_df):
    """
    El export cierre tiene metadata arriba.
    Buscamos la fila donde aparece Ticker, Especie o Symbol.
    """

    max_rows = min(len(raw_df), 30)

    candidates = {x.upper() for x in TICKER_CANDIDATES}

    for idx in range(max_rows):
        row_values = [str(x).strip().upper() for x in raw_df.iloc[idx].tolist()]
        if any(x in candidates for x in row_values):
            return idx
...
def pick_column(df, candidates):
    """
    Busca columna por nombre exacto, ignorando espacios alrededor y mayúsculas.
    """
